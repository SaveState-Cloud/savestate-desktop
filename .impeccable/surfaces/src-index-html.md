...
STORY
Cloud - Personal is always present and cannot be disconnected. Every connected B2, R2, S3-compatible, MinIO, or external-drive folder becomes another vault. Existing profiles remain attached to their existing destination.
...
FORM
The active vault is a persistent app context, not a card inside Personal. The manage view is secondary and contains the inline connect form. External drives choose an existing dedicated folder and do not show cloud-only credentials. A missing drive is labeled plainly and can be re-found by selecting the original marked folder. Source/schedule editing retains the familiar modal but its vault is fixed by the active context. Managed-only pages disappear while a custom vault is selected.
